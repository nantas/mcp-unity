...
usage() {
  cat <<'EOF'
Usage:
  scripts/install-package-snapshot.sh [--force] [--skip-gitignore] <unity-project-path>

Description:
  Copies a clean mcp-unity package snapshot into the target Unity project's
  Packages/ directory for personal local use without Package Manager.
...
if [[ ! -f "${SOURCE_ROOT}/Server~/build/index.js" ]]; then
  echo "error: Server~/build/index.js is missing. Run 'cd ${SOURCE_ROOT}/Server~ && npm install && npm run build' first." >&2
  exit 1
fi
...
rsync -a --delete \
  ...
  --exclude 'Server~/tsconfig.tsbuildinfo' \
...
Notes:
  - This is a copied snapshot, not a symlink.
  - The target project .gitignore now ignores /Packages/${PACKAGE_NAME} unless --skip-gitignore was used.
  - Re-run this script after future source changes if you want to refresh the installed snapshot.
EOF
